"""Function approximators. R01 starts with a dependency-free linear Q model."""
...
class LinearQNetwork:
    ...
    @classmethod
    def load(cls, path: Path) -> "LinearQNetwork":
        data = np.load(path)
...
def build_network(config: ExperimentConfig, input_dim: int) -> LinearQNetwork:
    if config.network == "linear_q":
        return LinearQNetwork(input_dim)
    raise NotImplementedError(f"Network {config.network!r} has not been implemented yet.")
