# Bomberman M3 手工特征线 —— 基线为 ε≡0 @ 10000 局

> **2026-08-30：基线换了两次，最终定为 ε≡0。本文标题里的 `R02_9` 是历史名字。**
>
> **锦标赛的对手是其他小组训练出来的 agent**——不是 `rule_based_agent`，
> 也不是我们自己的模型。这一条决定了下面的一切。
>
> | 候选 | 打 3× `rule_based`（35 次重复） | 打陌生对手 |
> |---|---:|---|
> | `R02_9` seed 1001 | 3.8790 | 金币最低一档 |
> | **ε≡0 seed 1001（提交）** | **3.8441** | **金币在四种对手上全部第一** |
> | Rainbow seed 1005 | **7.1270** | **击杀优势归零**，金币低于基线 |
>
> Rainbow 打 `rule_based` 的 +2.16 分里 95% 是击杀，而 §7.43 测出
> **换成任何没见过的对手，那 4.91 倍的击杀优势变成 0.96 倍 —— 归零而不是衰减。**
> 它赢的另一格是打它自己的复制品，同样不能外推。
> **金币是通用的，击杀是对手特异的**，而 ε≡0 在金币上四种对手全赢。
>
> **⚠️ 2026-08-31：上表「打陌生对手」那一列已被 §7.45 撤回。**
> 它是 3 个棋盘种子测的，而那些对手是确定性的——真正的样本量就是那 3 个。
> 加到 12 个之后：**击杀不是「归零」而是掉到 1.1–1.9 倍**，
> 而**分数上 ε≡0 并没有赢 Rainbow**（去掉镜像那格 t=+1.32 / +1.68，都不可分辨）。
> **提交选择因此重新打开，见 §7.45.3。** 唯一在 12 种子下仍然稳的是金币：
> ε≡0 在三个陌生对手上全部更高，两个可分辨。
> 比赛口径（`rule_based`）的 35 次重复数字不受影响。
>
> 新基线配方见 `docs/01` §7.44，迁移判定见 §7.43，ε≡0 的来历见 §7.42。
> run 目录 `runs/m3_noisy_10000_vs3rb_20260830`
> （名字里的 `noisy` 是 §7.42.1 那个 bug 留下的历史包袱，
> 它实际训练的是 `R02_9` 配方 + E12，checkpoint 里没有任何 σ 参数）。
