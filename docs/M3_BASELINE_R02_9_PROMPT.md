...
> `R02_11`（Rainbow，10000 局）在比赛口径上十次重复 **5.8149**，
> 对 `R02_9` 的 3.6371 是 **+2.1778，t = +32.59** —— §7.20 建立重复测量口径以来
> **第一个 `score` 可分辨为正的臂**。
> **增益全部来自击杀**（0.108 → 0.531，`t = +48.29`），`coins`、`suicides`、
> `coins_share` 三项全部不可分辨。**详见 `docs/01` §7.39。**
> 本文下面关于 `R02_9` 的配方与"已关闭方向"仍然有效——
> 它们描述的是**金币维度**，而那一维确实已经采满（§7.34）；
> Rainbow 打开的是**分数的另一半**。
> 提交模型的最终口径待 `rep_rainbow_n35` 补齐 35 次重复后确定。
