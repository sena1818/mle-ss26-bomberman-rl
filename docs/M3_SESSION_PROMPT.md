...
| `M3.3XL` | `m3_3xl_r02_3_a06_e01_t02_n5_v3_5000.json` | 5000 局：loot-crate **34.276**、suicides **0.249**、G2 6/6；但**比赛设置跌到 1.787**（§7.18） |
...
- **把对手训练当默认**：§7.15.3 / §7.19，score 上只差 6.7%（落在噪声里），
  但**存活率 3.9% → 17.4%（4.5×）、金币份额 18.6% → 22.0%（+18%）**。
  **若目标是比赛成绩，对手训练是值得的**；代价是单人 classic 从 5.433 掉到 1.724。
  **对手评估必须并列报告 `coins_share`**——score 把击杀和总币数波动混在一起，分辨率差。

## 当前诊断出的瓶颈（这是主线）

§7.16.5 / §7.18.4：**剩下的自炸死亡是值函数问题，不是表示问题。**
在 loot-crate solo 上 `v3` 的关键岔路分辨率已是 **0% 不可分辨（完美区分）**，
表示在训练任务上不再是瓶颈；`classic` + 对手上仍有 34.7%。
...
| # | 改动 | 依据 | 备注 |
|---|---|---|---|
| ~~1~~ | ~~预算 2000 → 5000~~ | **已做完，§7.18：score 只 +5.1%，比赛设置反而变差** | **预算线接近用尽，不要再加** |
...
| 5 | **把 `E01` 改成按绝对回合数定义**（`hold_rounds` 而非 `hold_fraction`） | §7.18.2：同配方同 seed，hold 400 vs 1000 在第 2000 局差 **2.7 倍**；**当前任何预算比较都同时在动探索时长** | 做了它，预算线才能重新测 |

**现在的第一优先级是第 2 项（`double_dqn`）**：§7.18.4 证明「看得见出口却选 WAIT」
在 2000 → 5000 局里只从 32 降到 24，占死因的比例反而从 7.4% 升到 11.4%——**训练治不好它**。
